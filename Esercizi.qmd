...
3.  Si sovraimponga, al grafico precedente, una linea verticale in corrispondenza del punto $-3$ ed una linea orizzontale in corrispondenza del punto $f(-3)$.
...
2.  Si utilizzi il vettore `v` per determinare il numero di interi tra 1 e 250 che sono maggiori di 97 e che sono numeri dispari.
...
2.  Si costruisca una matrica a piacere con cui verificare che il codice prodotto produca lo stesso risultato della funzione `t()`.
...
2.  Si scriva una funzione che normalizzi il vettore (cioè scalare il vettore in modo tale che i suoi elementi assumano valori in $[0,1]$).
...
Si implementi una funzione che riceve due interi $n$ e $k$ e restituisca una [tavola pitagorica](https://it.wikipedia.org/wiki/Tavola_pitagorica) di dimensione $n \times k$.
...
2.  In quanti modi è possibile dividere un insieme di 10 oggetti utilizzando 5 gruppi?
...
1.  Quante professioni sono state studiate? Quante variabili sono state raccolte?
...
Si carichino in `R` i dati contenuti nel file `Pokemon.csv`, scaricabili al seguente [link](https://robertoascari.github.io/data/Pokemon.csv). Il dataset contiene dati circa 800 Pokémon e le seguetni variabili:
...
1.  Si ispezioni la variabile `Legendary`. Come mai non è di classe `logi`, pur avendo modalità associabili a "Vero" e "Falso"? Si sovrascriva la variabile con una sua versione `logi`.
...
2.  Si crei un dataset `pkmn_sub` contenente i dati dei Pokémon appartenenti alla prima o alla seconda generazione. D'ora in poi, si faccia riferimento a questo `data.frame`.
...
3.  Si codifichino in `factor` le variabili che si ritiene sensato lo siano.
...
4.  Come si distribuiscono i Pokémon leggendari tra le due generazioni considerate? Si misuri il grado di connessione con un opportuno indice.
...
5.  Si calcoli, tramite la sua definizione, il valore della covarianza tra `Attack` e `Defense`. Si confronti il risultato ottenuto con quello fornito dalla funzione `cov()`.
...
6.  Si ottengano i boxplot relativi alla distribuzione di `Attack` condizionata per `Type1`.
...
7.  Si proponga un modo per ottenere le medie condizionate di `Attack` entro ogni modalità di `Type1`.
...
8.  Si costruisca un `data.frame` contenente esclusivamente le variabili quantitative di `pkmn_sub`. Si utilizzi questo `data.frame` per costruire la matrice di correlazione (arrotondata alla terza cifra decimale) e la matrice di diagrammi a dispersione. Si colorino i punti dei diagrammi a dispersione sulla base del valore di `Legendary`.
...
3.  Si vuole indagare l'eterogeneità della variabile `month`, per valutare quindi se le partenze possono ritenersi equiripartite nei mesi dell'anno. Si calcolino quindi l'indice di Gini normalizzato e l'indice di Shannon normalizzato.
...
6.  Si calcoli il ritardo totale medio per compagnia aerea (in inglese carrier). Quale compagnia sembra essere la più efficiente, sulla base di questa piccola analisi?
...
8.  In future analisi legate al turismo, si sarà interessati a studiare i soli voli decollati nel periodo estivo. A tal fine, si costruisca un dataset `flights_summer` riguardanti i voli partiti dal 1 giugno al 31 agosto.
...
Si consideri il dataset `olympics.RData` (scaricabile al seguente [link](https://robertoascari.github.io/data/olympics.csv)), contenente dati relativi alle Olimpiadi di Londra 2012. In particolare, il dataset contiente le seguenti variabili:
...
Si importi il dataset \texttt{animals.csv}, scaricabile al seguente [link](https://robertoascari.github.io/data/animals.csv), contenente informazioni su 38 specie di animali. In particolare si hanno le seguenti variabili:
...
2.  Si calcoli una nuova variabile `sonno`, rappresentante il numero totale di ore di sonno giornaliere.
...
7.  Si ritiene possibile proporre un grafico per studiare la relazione tra `sonno` e `peso_cerv`, tenendo conto anche della presenza della variabile `preda`? Se si, si riporti tale grafico e lo si commenti. Altrimenti si motivi il perché non lo si ritiene possibile.
...
3.  Si ricorda che se $X_1, \dots, X_g$ sono i.i.d. e $X_j \sim N(0,1)$, allora $\displaystyle \sum_{j=1}^g X_j \sim \chi^2_g$. Si uti questo risultato teorico per ottenere un campione di `8000` valori da una $\chi^2_10$ sfruttando unicamente la funzione `rnorm()`.
...
4.  Nell'inferenza è spesso necessario ricavare ricavare dei quantili di una $\chi^2_g$. Si utilizzi R per ottenere il quantile di ordine 0.95 di una $\chi^2_{10}$ in due modi differenti, uno esatto ed uno approssimato.
...
1.  Si calcoli il valore della divergenza di Kullback-Leibler di $X_1$ da $X_2$ considerando $X_1$ e $X_2$ distribuite come Binomiali di parametro $n = 20$ e probabilità di successo $\theta_1 = 0.2$ e $\theta_2 = 0.1$.
...
3.  Si calcoli la stessa quantità facendo variare $\theta_2 \in \{0.05, 0.1, 0.2, 0.3, 0.5, 0.7, 0.9\}$. Da questi risultati è possibile intuire cosa misura questo indice?
...
$\displaystyle f(y; \alpha, \beta) = \alpha \beta y^{\beta - 1} e^{-\alpha y^\beta}$, se $y > 0$, dove $\alpha$ e $\beta$ sono due parametri positivi.
...
5.  Si verifichi che la funzione `my_rweibull` permette di estrarre valori pseudo-casuali dalla funzione di densità in analisi.
...
2.    Si proponga un modo per verificare se i metodi proposti sembrano generare valori proprio dalla distribuzione desiderata.
...
3.    Si usi uno dei campioni ottenuti per approssimare il valore atteso, il momento secondo e la varianza di $X$.
```{r}
#| echo: true
#| eval: false
#| code-fold: true
...
1. Si considerino $\mu$ e $\sigma^2$ entrambi ignoti. Si fornisca una stima per entrambi i parametri. Qual è l'errore commesso dai due stimatori, in questo caso?
...
2. Siano $Y_1, \dots Y_D$ delle variabili casuali indipendenti tali che $Y_d \sim Gamma(\alpha_d, 1)$. Definendo $\displaystyle Y^+ = \sum_{d=1}^D Y_d$, si può dimostrare che $\displaystyle (Y_1 / Y^+, \dots, Y_D/Y^+)^\intercal \sim Dirichlet(\boldsymbol{\alpha})$. Si utilizzi questa relazione per costruire un vettore `sample_dir` di valori pseudo-casuali da una Dirichlet con $D = 5$ e $\boldsymbol{\alpha} = (4, 2, 10, 5, 3)^\intercal$.
...
3. Si considerino le funzioni $f(x) = \sqrt{|x|}$ e $g(x) = x^2 + 1$. Si calcoli il valore dell'integrale con gli approcci del punto 1 e punto 2 li si confronti con la funzione `integrate`.
...
1. Si ottengano 4 realizzazioni del campione di ampiezza $n \in \{10, 50, 100, 1000\}$, salvandoli dentro una lista.
...
3. Si implementi uno studio di simulazione per verificare empiricalmente la convergenza in distribuzione dello stimatore MV. Per ciascuna delle 4 numerosità campionarie, si considerino 10000 repliche.
...
Si consideri un campione casuale di ampienzza $n$ estratto da una popolazione Normale di parametri $\mu$ e $\sigma^2$.
Si scelgano a piacere dei valori per gli ignoti parametri e si generi un campione di ampiezza $n = 35$.
...
1. Si consideri $sigma^2$ noto e pari al vero valore scelto. Si implementi la funzione `loglik_mu(mu, y)` che permetta di calcolara la funzione di log-verosimiglianza. Si rappresenti graficamente tale funzione in un opportuno intervallo.
...
3. Si consideri adesso anche il parametro $\sigma^2$. Si implementi la funzione `loglik_mu_sigma2(mu, y)` che permetta di calcolara la funzione di log-verosimiglianza per entrambi i parametri. Si rappresenti graficamente questa funzione.
```{r}
#| echo: true
#| eval: false
#| code-fold: true
...
log_lik_values <- matrix(log_lik_values, nrow = length(griglia_mu), ncol = length(griglia_sigma2), byrow = F)
...
var(y)
```
5. Si riportino, sul grafico ottenuto al punto 3, due punti corrispondenti al vero valore dei parametri e alla stima MV. Si commenti il grafico ottenuto.
...
Sia $\textbf{y} = (y_1, \dots, y_n)^\intercal$ un campione di realizzazioni i.i.d. da una variabile casuale Poisson di parametro $\theta >0$. La funzione di log-verosimiglianza associata a tale campione, a meno di una costante additiva, è specificata come 
$$l(\theta; \textbf{y}) = -n\theta + \log(\theta) \sum_{i=1}^n y_i.$$
...
2. D'ora in avanti si consideri il campione $\yv = (5, 10,  7,  7,  9,  9,  5)^\intercal$. Quanto vale la funzione di log-verosimiglianza nel punto $\theta = 10$?
...
Sia $Y_1, \dots, Y_n$ un campione casuale estratto da una Poisson$(\lambda)$. È noto che il parametro $\lambda >0$ di una Poisson coincide con il suo valore atteso e con la sua varianza.
